        ...
        <form action="{{ $part->exists ? route('admin.exams.update',$part) : route('admin.exams.store') }}"
                ...
                <div class="card card-body bg-light p-3">
                    ...
                    <div class="form-group mb-0">
                        ...
                        @if($part->exists && $part->vdo_path)
                            <div class="mb-2">
                                <video src="{{ Storage::url($part->vdo_path) }}" controls style="max-height:120px;max-width:100%;" class="rounded"></video>
                                <p class="text-muted small mt-1">ไฟล์ปัจจุบัน: {{ basename($part->vdo_path) }}</p>
                            </div>
                        @endif
                        <input type="file" name="vdo_file" class="form-control-file @error('vdo_file') is-invalid @enderror"
                               accept=".mp4,.webm,.mov">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="number" name="post_test_points" class="form-control" min="0"
                               value="{{ old('post_test_points', $part->post_test_points ?? 8) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                @if($part->exists && $part->banner_image)
                    <div class="mb-2">
                        <img src="{{ Storage::url($part->banner_image) }}" style="height:80px;object-fit:cover;" class="img-thumbnail">
                    </div>
                @endif
                <input type="file" name="banner_image" class="form-control-file" accept=".jpg,.jpeg,.png">
